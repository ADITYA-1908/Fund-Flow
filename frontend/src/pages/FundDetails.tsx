...
const FundDetails: React.FC = () => {
  ...
  const fetchFundData = async () => {
    try {
      ...
      try {
        // First try: Backend proxy route
        response = await axios.get(`/api/proxy/mf/${schemeCode}`);
        console.log("✅ Backend proxy API success");
      } catch (proxyError) {
        console.warn(
          "❌ Backend proxy failed, trying direct backend API:",
          proxyError
        );

        try {
          // Second try: Backend direct route
          response = await axios.get(`/api/mf/${schemeCode}`);
          console.log("✅ Backend direct API success");
        } catch (backendError) {
          console.warn(
            "❌ Backend direct API failed, trying external MFAPI:",
            backendError
          );

          // Third try: Direct MFAPI call
          response = await axios.get(`https://api.mfapi.in/mf/${schemeCode}`);
          console.log("✅ External MFAPI success");
        }
      }
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            The fund with scheme code {schemeCode} could not be loaded.
          </p>
          <button onClick={() => navigate("/")} className="btn-primary">
            Go back to search
          </button>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            NAV data is not available for this fund.
          </p>
          <button onClick={() => navigate("/")} className="btn-primary">
            Go back to search
          </button>
